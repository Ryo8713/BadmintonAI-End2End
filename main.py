...
if __name__ == '__main__':
    ...
    # TrackNet
    print("\n[Message] Start trajectory prediction\n")
    for clip in os.listdir(RALLY_OUTPUT):
        print(clip)
        clip_path = Path(f'{RALLY_OUTPUT}{clip}')
        traj_csv_file = predict_traj(clip_path, TRAJ_OUTPUT)
        df = pd.read_csv('traj/clip_1_ball.csv', encoding="utf-8")
        smooth(traj_csv_file, df)
    print("\n[Message] Trajectory prediction finished\n")